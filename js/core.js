...
function saveImageToDB(storeName, id, dataUrl) {
    return openDB().then(function(database) {
        return new Promise(function(resolve, reject) {
            ...
            store.put({ id: id, data: dataUrl, time: Date.now() });
...
function saveData() {
    ...
    try {
        ...
        console.log('数据已保存，聊天记录数:', appData.chatHistory.length, '大小:', jsonStr.length);
    } catch(e) {
...
function loadData() {
    return new Promise(function(resolve) {
        ...
        if (saved) {
            try {
                ...
                if (p && typeof p === 'object') {
                    console.log('从 localStorage 加载数据成功，聊天记录数:', p.chatHistory?.length);
                    ...
                    if (typeof p.forumReplies === 'object') appData.forumReplies = p.forumReplies;
        ...
        if (typeof appData.forumReplies !== 'object') appData.forumReplies = {};
...
function uploadToMorePanel() {
    ...
    compressImage(file, 400, 400, 0.5).then(function(url) {
        ...
        return saveImageToDB('images', id, url).then(function() {
            ...
            return saveData();
        }).then(function() { renderMoreImages(); showToastLong('表情包上传成功！', 3500); });
...
function openReplyModal() {
    ...
    var html = '<h4>自定义回复</h4><button class="btn-sm" onclick="addNewGroup()" style="margin-bottom:10px;">+ 新建分组</button><div style="max-height:50vh;overflow-y:auto;">' + groupsHtml + '</div><button class="btn-close" onclick="closeModal(\'subOverlay\')" style="margin-top:12px;">关闭</button>';
    openSubModal(html);
}
...
function delReplySingle(g, r) { appData.replyGroups[g].replies.splice(r,1); saveData(); openReplyModal(); showToast('已删除'); }
...
function openBackupModal() {
    ...
    html += '<div style="margin-top:12px;"><button class="btn-sm danger-sm" onclick="clearChatHistory()">清除聊天记录</button></div>';
...
function exportFullAsFile() {
    ...
    Promise.all(emojiPromises).then(function(emojiData) {
        var backupData = {
            ...
            books: appData.books, wheelItems: appData.wheelItems, wheelHistory: appData.wheelHistory, vdWordBank: appData.vdWordBank
        };
        var timestamp = new Date().toISOString().replace(/[:.]/g, '-').substring(0, 19);
        downloadJSONFile('chat_app_backup_' + timestamp + '.json', backupData);
        closeModal('subOverlay'); showToast('备份文件已下载');
    });
}
...
function fallbackCopy(text, label) {
    ...
    try { document.execCommand('copy'); showToastLong(label + '已复制到剪贴板', 3000); } catch(e) { showToast('复制失败'); }
...
function importDataFile() {
    ...
    reader.onload = function(e) {
        try {
            ...
            // 恢复头像
            if (data.myAvatar && data.myAvatar.length > 100) {
                var myId = 'avatar_me_' + Date.now();
                saveImageToDB('avatars', myId, data.myAvatar).then(function() {
                    appData.myAvatarId = myId;
                    appData.myAvatar = data.myAvatar;
                });
            }
            if (data.otherAvatar && data.otherAvatar.length > 100) {
                var otherId = 'avatar_other_' + Date.now();
            ...
            closeModal('subOverlay');
            showToastLong('数据导入成功！共 ' + appData.chatHistory.length + ' 条聊天记录', 3000);
            
            // 验证保存成功
            var verify = localStorage.getItem(STORAGE_KEY);
            if (verify) {
                console.log('验证成功，localStorage 大小:', verify.length, '聊天记录数:', appData.chatHistory.length);
            }
        } catch(err) { 
            console.error(err);
...
function showToastLong(msg, duration) {
    var t = document.getElementById('toast'); if (!t) return;
    t.textContent = msg; t.style.display = 'block';
    clearTimeout(t._timeout); t._timeout = setTimeout(function() { t.style.display = 'none'; }, duration || 3000);
}
...
window.addEventListener('pagehide', function() { saveData(); });
setInterval(function() { saveData(); }, 10000);
...
function sendTransferToOther() {
    ...
    saveData(); closeModal('subOverlay'); showToast('已转账 ¥' + amount);
}
function openTransferModal() {
    ...
    ['5.20','13.14','52.00','131.40','520.00'].forEach(function(a){ html += '<button class="btn-sm outline" onclick="document.getElementById(\'transferAmountInput\').value=\''+a+'\'">¥'+a+'</button>'; });
    html += '</div><div class="btn-row" style="margin-top:10px;"><button class="btn-sm" onclick="sendTransferToOther()">确认转账</button><button class="btn-sm outline" onclick="receiveTransferFromOtherModal()">收转账</button></div>';
...
initApp().then(function(){ 
    console.log('启动成功，当前聊天记录数:', appData.chatHistory.length);
    console.log('localStorage 中的聊天记录数:', (function() {
        try { var d = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}'); return d.chatHistory?.length || 0; } catch(e) { return 0; }
    })());
});
